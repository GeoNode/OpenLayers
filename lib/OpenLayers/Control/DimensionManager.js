...
OpenLayers.Control.DimensionManager = OpenLayers.Class(OpenLayers.Control, {
    ...
    initialize : function(options) {
        ...
        if(this.model){
            if (!(this.model instanceof OpenLayers.Dimension.Model)) {
                this.model = new OpenLayers.Dimension.Model(this.model);
            }
            this.syncToModel();
        }
        if(this.agents) {
            for(var i = 0, len = this.agents.length; i < len; i++) {
                ...
                // we have been supplied with config instead
                if (!(agent instanceof OpenLayers.Dimension.Agent)) {
                    var config = agent;
                    config.dimensionManager = this;
                    var agentClass = config.agentClass;
                    delete config.agentClass;
                    agent = new OpenLayers.Dimension.Agent[agentClass](config);
                }
