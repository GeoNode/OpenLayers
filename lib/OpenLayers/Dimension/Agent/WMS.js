...
OpenLayers.Dimension.Agent.WMS = OpenLayers.Class(OpenLayers.Dimension.Agent, {
    ...
    initialize : function(options) {
        ...
        if(this.layers) {
            for(var i = 0, len = this.layers.length; i < len; i++) {
                ...
                // TODO, ideally this check should not be at this level, but
                // at the application level
                if (window.location.href.match(/view|new/)===null &&
                  OpenLayers.Util.getParameters().prebuffer !== "false") {
                    this.preBuffer(layer);
                }
                layer.events.on({
    ...
    preBuffer: function(layer) {
        ...
        if (this.model.values) {
            len = this.model.values.length;
        } else if (this.model.resolution) {
            len = (this.model.range[1] - this.model.range[0])/this.model.resolution;
        }
        clone.events.on({
            'loadend': function() {
                i++;
                if (i<len) {
                    this.doBufferStep(clone, i, len);
                } else {
        ...
        this.count = 0;
        this.doBufferStep(clone, 0, len);
    },

    doBufferStep: function(clone, i, len) {
        this.count++;
        var value = null;
        if (this.model.values) {
        ...
        var progress = (this.count)/(this.layers.length*len);
        this.model.events.triggerEvent("prebuffer", {progress: progress});
        if (value !== null) {
